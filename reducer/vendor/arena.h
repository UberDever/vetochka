...
#else
#error "No portable C99 alignof available"
#endif
#endif
...
void* arena_alloc_aligned(Arena* arena, size_t size, unsigned int alignment) {
  ...
  if (arena == NULL || arena->region == NULL) { return NULL; }
...
void arena_destroy(Arena* arena) {
  ...
  if (arena->region != NULL) { ARENA_FREE(arena->region); }
...
Arena_Allocation* arena_get_allocation_struct(Arena* arena, void* ptr) {
  ...
  if (arena == NULL || ptr == NULL) { return NULL; }

  current = arena->head_allocation;
  while (current != NULL) {
    if (current->pointer == (char*)ptr) { return current; }
